## Hello, moi c’est Emeline Delobel 👋

👩‍💻 **Software Engineer** | Full-Stack & DevOps  
💡 J’aide les entreprises et entrepreneurs à concevoir des solutions web **modernes, robustes et évolutives**.

---

## 🧰 Mes expertises

- Développement **Full-Stack** (Java, TypeScript, JavaScript, PHP)
- Conception d'applications web performantes et sécurisées
- **CI/CD**, automatisation & déploiement cloud (Docker, GitHub Actions)
- Travail en équipe agile & code maintenable

---

### 🧰 Outils & langages

![Java](https://img.shields.io/badge/Java-%23ED8B00.svg?style=flat&logo=java&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-%23323330.svg?style=flat&logo=javascript)
![TypeScript](https://img.shields.io/badge/TypeScript-%23007ACC.svg?style=flat&logo=typescript&logoColor=white)
![PHP](https://img.shields.io/badge/PHP-%23777BB4.svg?style=flat&logo=php&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-%23339933.svg?style=flat&logo=node.js&logoColor=white)
![Vue.js](https://img.shields.io/badge/Vue.js-%2335495e.svg?style=flat&logo=vuedotjs)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-%2306B6D4.svg?style=flat&logo=tailwindcss)
![Docker](https://img.shields.io/badge/Docker-%230db7ed.svg?style=flat&logo=docker&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-%232671E5.svg?style=flat&logo=githubactions&logoColor=white)

---

![Langages utilisés](https://github-readme-stats.vercel.app/api/top-langs/?username=EmelineDe&layout=compact&theme=radical)


## 🚀 Projets récents

🔹 [**GoWebApp_Front**](https://github.com/EmelineDe/GoWebApp_Front) – Front-end d'une application Vue3/Tailwind  
🔹 [**GoWebApp_Back**](https://github.com/EmelineDe/GoWebApp_Back) – API Node.js avec TypeScript  
🔹 [**Blog_Voyage**](https://github.com/EmelineDe/Blog_Voyage) – Application PHP PDO / AJAX

👉 Découvre aussi mon [**portfolio**](https://github.com/EmelineDe/Portfolio)

---

## 📫 Me contacter

📍 France  
📧 delobel.emeline@gmail.com 
🌐 [LinkedIn](https://linkedin.com/in/emeline-d-6915a417a)
📁 Mon [**portfolio**](https://portfolio-emelinedelobel.fr)

---

Merci pour ta visite ! 🙌  
N’hésite pas à jeter un œil à mes projets ou à me contacter pour collaborer 🚀
